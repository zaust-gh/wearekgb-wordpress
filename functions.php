...
function boilerplate_continue_reading_link() {
	return ' <a class="readMore" href="'. get_permalink() . '">' . __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'boilerplate' ) . '</a>';
}
...
function create_my_post_types() {
	register_post_type( 'sitecontent',
		array(
			'labels' => array(
			'name' => __( 'Sitecontent' ),
			'singular_name' => __( 'Sitecontent' ),
			'add_new' => __( 'Add New' ),
			'add_new_item' => __( 'Add New Sitecontent' ),
			'edit' => __( 'Edit' ),
			'edit_item' => __( 'Edit Sitecontent' ),
			'new_item' => __( 'New Sitecontent' ),
			'view' => __( 'View Sitecontent' ),
			'view_item' => __( 'View Sitecontent' ),
			'search_items' => __( 'Search Sitecontent' ),
			'not_found' => __( 'No Sitecontent found' ),
			'not_found_in_trash' => __( 'No Sitecontent found in Trash' ),
			'parent' => __( 'Parent Sitecontent' ),
		),
			'public' => true,
			'show_ui' => true,
	...
	register_post_type( 'chronicle',
		array(
			'labels' => array(
			'name' => __( 'chronicles' ),
			'singular_name' => __( 'chronicle' ),
			'add_new' => __( 'Add New' ),
			'add_new_item' => __( 'Add New chronicle' ),
			'edit' => __( 'Edit' ),
			'edit_item' => __( 'Edit chronicle' ),
			'new_item' => __( 'New chronicle' ),
			'view' => __( 'View chronicle' ),
			'view_item' => __( 'View chronicle' ),
			'search_items' => __( 'Search chronicles' ),
			'not_found' => __( 'No chronicles found' ),
			'not_found_in_trash' => __( 'No chronicles found in Trash' ),
			'parent' => __( 'Parent chronicle' ),
		),
			'public' => true,
			'show_ui' => true,
			'publicly_queryable' => true,
...
function admin_init(){
  	add_meta_box("media_meta", "Media Information", "media_meta", "video", "normal", "low");
	add_meta_box("slider", "Is the post for the slider?", "slider", "post", "normal", "low");
}

function media_meta() {
  global $post;
  $custom = get_post_custom($post->ID);
  $videolink = $custom["videolink"][0];
  ?>
  <p><label>Video link:</label><br />
  <input name="videolink" value="<?php echo $videolink; ?>"></input></p>
  <?php
}
...
function slider() {
  global $post;
  $custom = get_post_custom($post->ID);
  $slider = $custom["isforslider"][0];
  ?>
  <p><label>Would you like the post to appear in the slider?:</label>&nbsp;&nbsp;<input name="isforslider" type="checkbox" <?php if ($slider == 'on') {?> checked="yes" <?php } ?> /></p>
  <?php
}
...
function save_details(){
  ...
  update_post_meta($post->ID, "isforslider", $_POST["isforslider"]);
...
add_image_size( 'blogthumb', 85, 100, true );
add_image_size( 'blogslider', 430, 230, true );

// custom_type template redirect
  
  
function my_template_redirect() {
  if ( is_robots() || is_feed() || is_trackback() || is_single() || is_tag() ) {
